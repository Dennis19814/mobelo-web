...
import {
  CheckCircle, AlertCircle, TestTube, Edit2, Key, MessageSquare
} from 'lucide-react';
...
interface SettingsSmsSectionProps {
  appId: number;
  apiKey?: string;
  appSecretKey?: string;
}

export default function SettingsSmsSection({ appId, apiKey, appSecretKey }: SettingsSmsSectionProps) {
  ...
  const handleSaveConfiguration = async (config: CreateAppConfigDto | UpdateAppConfigDto, configId?: number) => {
    try {
      ...
      const effectiveConfigId = configId ?? twilioConfig?.id;
    ...
    <div className=" w-full max-w-full">
      ...
      <div className="mb-6">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-2 lg:gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-gray-900 mb-2">SMS Configuration</h1>
            <p className="text-sm md:text-base text-gray-600 break-words">Manage Twilio credentials for SMS OTP delivery</p>
          </div>
          <button
      ...
      <Suspense fallback={null}>
        {showTwilioModal && (
          <TwilioConfigModal
            ...
            existingConfig={twilioConfig || null}
            appId={appId}
          />
        )}
      </Suspense>
