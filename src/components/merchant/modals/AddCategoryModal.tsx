...
export function AddCategoryModal({
  ...
  const [displayMode, setDisplayMode] = useState<'image' | 'icon'>('image'); // Default to image mode
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (result) {
      ...
      setDisplayMode('image'); // Reset to default mode
      onClose();
    } else {
  ...
  const handleChangeImage = () => {
    ...
    if (fileInputRef.current) {
      // Clone the input to force reset
          ...
          <form onSubmit={handleFormSubmit} className="p-4 space-y-4 overflow-y-auto flex-1 min-h-0">
            ...
            {displayMode === 'icon' && (
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                ...
                {((formData.displayType === 'icon' && formData.iconUrl && formData.iconUrl !== 'lucide-react:Folder' && selectedIcon && selectedIcon.name !== 'Folder') || (formData.displayType === 'emoji' && formData.emojiUnicode && selectedEmoji)) ? (
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2 p-2 bg-white rounded-lg border border-orange-200">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-700 truncate">
                          {formData.displayType === 'icon' && selectedIcon ? selectedIcon.name : selectedEmoji?.name || 'Selected'}
                        </p>
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="px-4 py-2 text-sm font-medium bg-orange-600 text-white rounded-lg hover:bg-orange-700 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
