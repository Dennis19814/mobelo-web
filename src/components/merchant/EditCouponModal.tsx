...
export default function EditCouponModal({ coupon, onClose, onSuccess }: EditCouponModalProps) {
  ...
  // Initialize form data with existing coupon values
  const [formData, setFormData] = useState({
    ...
    applicationType: coupon.applicationType as ApplicationType,
    ...
    customerEligibility: (coupon.customerEligibility || 'all') as CustomerEligibility,
    status: coupon.status as CouponStatus,
    isStackable: coupon.isStackable || false,
    priority: coupon.priority || 0,

    // Target scope
    productIds: coupon.targetScope?.productIds || [] as number[],
    categoryIds: coupon.targetScope?.categoryIds || [] as number[],
    brandIds: coupon.targetScope?.brandIds || [] as number[],
    excludeProductIds: coupon.targetScope?.excludeProductIds || [] as number[],
    excludeCategoryIds: coupon.targetScope?.excludeCategoryIds || [] as number[],
    excludeBrandIds: coupon.targetScope?.excludeBrandIds || [] as number[],

    // Buy X Get Y config
    buyQuantity: coupon.buyXGetYConfig?.buyQuantity,
    getQuantity: coupon.buyXGetYConfig?.getQuantity,
    getDiscountType: (coupon.buyXGetYConfig?.getDiscountType || 'free') as 'free' | 'percentage' | 'fixed',
    getDiscountValue: coupon.buyXGetYConfig?.getDiscountValue,
    targetProductIds: coupon.buyXGetYConfig?.targetProductIds || [] as number[],

    // Specific users
    specificUserIds: coupon.specificUserIds || [] as number[],
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const couponData: UpdateCouponData = {
      code: formData.code,
      name: formData.name,
      description: formData.description || undefined,
      discountType: formData.discountType,
      discountValue: toNumber(formData.discountValue) as number,
      maxDiscountAmount: toNumber(formData.maxDiscountAmount),
      applicationType: formData.applicationType,
      minPurchaseAmount: toNumber(formData.minPurchaseAmount),
      minQuantity: toNumber(formData.minQuantity),
      maxUsageTotal: toNumber(formData.maxUsageTotal),
      maxUsagePerUser: toNumber(formData.maxUsagePerUser),
    ...
    if (formData.applicationType !== 'entire_order') {
      ...
      const hasScopeData = Object.values(scopePayload).some((value) => Array.isArray(value) ? value.length > 0 : value !== undefined);

      if (hasScopeData) {
        couponData.targetScope = scopePayload;
      }
    }

    if (formData.discountType === 'buy_x_get_y' && toNumber(formData.buyQuantity) && toNumber(formData.getQuantity)) {
      couponData.buyXGetYConfig = {
        buyQuantity: toNumber(formData.buyQuantity)!,
        getQuantity: toNumber(formData.getQuantity)!,
        getDiscountType: formData.getDiscountType,
        getDiscountValue: toNumber(formData.getDiscountValue),
    ...
    try {
      const response = await apiService.updateCoupon(coupon.id, couponData);

      if (response.ok) {
        onSuccess();
        onClose();
      } else {
        const errorMsg = response.data?.message || 'Failed to update coupon';
        setError(errorMsg);
      }
    } catch (err: any) {
      console.error('Failed to update coupon:', err);
      const errorMsg = err?.message || 'An unexpected error occurred while updating the coupon';
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
          ...
          <div className="border-b pb-3">
            ...
            {formData.applicationType !== 'entire_order' && (
              <div className="mt-2 p-3 bg-gray-50 rounded-lg space-y-2">
                <h4 className="text-sm font-medium mb-2">Target Scope</h4>
