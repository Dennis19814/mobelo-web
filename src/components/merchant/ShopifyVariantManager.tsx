...
import { Plus, Trash2, GripVertical, AlertCircle, Check, X, Search, Filter, ChevronDown, ChevronUp, Image as ImageIcon } from 'lucide-react'
...
function DraggableOption({
  ...
  validationErrors: Record<string, { name?: boolean; values?: boolean }>
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className={`flex-1 px-3 py-1.5 border rounded-lg text-sm ${
                  validationErrors[option.id]?.name ? 'border-red-300 bg-red-50' : 'border-gray-300'
                } focus:ring-2 focus:ring-orange-500 focus:border-orange-500`}
                placeholder="e.g., Size"
              />
            </div>
            {validationErrors[option.id]?.name && (
          ...
          <div className="flex items-center justify-between pt-2 border-t border-gray-200">
            ...
            <button
              ...
              disabled={!option.name.trim() || option.values.filter(v => v.trim()).length === 0}
...
export default function ShopifyVariantManager({
  ...
  const handleDone = (optionId: string) => {
    ...
    const errors: { name?: boolean; values?: boolean } = {}
    if (!option.name.trim()) {
      errors.name = true
    }
      ...
      <div className="bg-white  ">
        ...
        {options.length === 0 ? (
          ...
          <>
            ...
            {/* Add Another Option Button - Only show when there are saved (non-editing) options */}
            {options.some(opt => !opt.isEditing) && (
              <button
                type="button"
                onClick={handleAddOption}
                disabled={editingOptionId !== null}
